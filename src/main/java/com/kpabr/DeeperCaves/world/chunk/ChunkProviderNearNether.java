...
public class ChunkProviderNearNether extends ChunkProviderDeeperCavesBase
{
    ...
    NoiseGeneratorOctavesOpenSimplex caveNoisePillar;
    ...
    double[] cavePillarNoiseField = new double[59392];
    ...
    public ChunkProviderNearNether(World par1World, long par2, boolean par4)
    {
        ...
        caveNoisePillar = new NoiseGeneratorOctavesOpenSimplex(new Random(this.worldObj.getSeed() + ((WorldProviderDeeperCaves)this.worldObj.provider).layer.seedOffset + 3), 1);
    }
    ...
    public void finalGenPass(int p_147422_1_, int p_147422_2_, Block[] p_147422_3_, byte[] p_147422_4_, BiomeGenBase[] p_147422_5_)
    {
        ...
        cavePillarNoiseField = caveNoisePillar.generateNoiseOctaves(cavePillarNoiseField, p_147422_1_*16, 0, p_147422_2_*16, 16, 102, 16, 0.05, 0.001, 0.05);
            ...
            for(int sculkZ = 0; sculkZ < 16; sculkZ++) {
                for(int sculkY = 0; sculkY < 102; sculkY++) {
                    ...
                    if(caveDensityNoiseFieldRegional[(sculkX * 16) + sculkZ] > 0)
                    {
                        ...
                        double caveVal = caveNoiseField[ind] * caveMod - (2.3 * rangeClamp);
                        if (caveVal > 0 && (b == DeeperBlocks.deepStone || b == DeeperBlocks.basalt)) {
                            double pillarVal = (Math.abs(cavePillarNoiseField[ind])-0.7)*12;
                            if(!(pillarVal > (caveVal > 0.25 ? 0 : -1*(2*(0.25-caveVal))))) {
